def getReleaseConfirmation() {
  ...
  def inputMessage = "Please provide release version for the Bona Fide User Service"
  getBuildVersion()
...
def getBuildVersion() {
  ...
  url: 'git@github.com:Ante-Meridiem/Bona-Fide-User-Service.git'
...
def fecthJarAndDockerFile() {
  ...
  try {
    ...
    script: '''sudo cp /home/ec2-user/Bona-Fide-User-Service/bona-fide-user-service.jar target'''
    sh label: 'copyDockerFile',
    script: 'sudo cp /home/ec2-user/Bona-Fide-User-Service/Dockerfile .'
  }
  catch(Exception e) {
...
def stopRunningContainer() {
  ...
  sshagent(['bonaFideDeploymentAccess']) {
    final String currentImageId = sh(script: 'ssh -o StrictHostKeyChecking=no ec2-user@13.126.97.24 docker ps -q -f name="^bona_fide_user_service_container$"', returnStdout: true)
         if(!currentImageId.isEmpty()){
             echo 'Stopping Current Container'
             try{
		sh 'ssh -o StrictHostKeyChecking=no ec2-user@13.126.97.24 docker stop bona_fide_user_service_container '
             }
             catch(Exception e){
	     ...
	     echo 'Renaming Current Container '
             try{
                sh 'ssh -o StrictHostKeyChecking=no ec2-user@13.126.97.24 docker rename bona_fide_user_service_container bona_fide_user_service_container_old '
             }
             catch(Exception e){
...
def runContainer(){
    ...
    def dockerRunCommand = "docker run -d -p 9004:9004 --name bona_fide_user_service_container docker4bonafide/${buildVersion}"
    sshagent(['bonaFideDeploymentAccess ']) {
        try{
...
def performHealthCheck(){
    ...
    final String url = 'http://ec2-13-126-97-24.ap-south-1.compute.amazonaws.com:9004/bona-fide-user-service/base/version'
...
def performCleanSlateProtocol() {
	...
	try{
		sh(script:'sudo rm -rf Bona-Fide-User-Service')
    		sh(script:'sudo rm -rf Bona-Fide-User-Service@tmp')
	}
	catch(Exception e){
